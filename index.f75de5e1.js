...
function copyColumn(tableData, index) {
    ...
    sections.forEach((section)=>{
        if (section) for(let i = 0; i < section.rows.length; i++){
            const row = section.rows[i];
            const cell = row.cells[index];
            if (cell) {
                const newCell = section === table.tHead || section === table.tFoot ? document.createElement("th") : document.createElement("td");
